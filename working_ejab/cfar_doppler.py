...
rmax = len(rxSignal[0])*c/fs/2 # Maximum Range
...
ax = axes[0]
ax.plot(getRank(rxSignal[0]),rxSignal[0])
...
#matchedFilter = np.conj(np.flip(chirp_signal('upchirp',BW,Tp,fs)))
matchedFilter = chirp_signal('downchirp',BW,Tp,fs)

#t = np.arange(-Tp/2,Tp/2,1/fs)
#chirp_ref = np.exp(1j*np.pi*BW/Tp * t**2)
#matchedFilter = np.conj(np.flip(chirp_ref))
#chirp_comp = fastconv(matchedFilter,chirp_ref)
...
# ax = axes[1]
# ax.plot(matchedFilter.real,label=r'$\Re(Chirp Ref(t))$')
# ax.plot(matchedFilter.imag,label=r'$\Im(Chirp Ref(t))$')
# ax.set_ylabel('Amplitude')
# ax.set_xlabel('Samples')
# ax.set_title('Filter')
# ax.legend(loc='upper right')
...
# ax = axes[0]
# ax.plot(chirp_comp.real,label=r'$\Re(Chirp Comp(t))$')
# ax.plot(chirp_comp.imag,label=r'$\Im(Chirp Comp(t))$')
# ax.set_ylabel('Amplitude')
# ax.set_xlabel('Samples')
...
for t in range(len(rxSignal)):
    ...
    comp = fastconv(rxSignal[t],matchedFilter)[len(matchedFilter)//2:len(rxSignal[t])+len(matchedFilter)//2]
...
ax = axes[1]
ax.plot(getRank(signal_comp[0]),np.abs(signal_comp[0]))
ax.set_ylabel('Abs value')
...
def calc_th_mti_sc(gain_mti,cfar_mti_sc):
    # convolucionar y quitar colas de convolucion
    ...
    targets_rank = getRank(dif_th_mti_sc)
    targets_rank = targets_rank[dif_th_mti_sc==2]
...
def update(val):
    ...
    th_cfar_mti_sc = calc_th_mti_sc(gain_slider.val,cfar_mti_sc)[0]
    dif_th_mti_sc = calc_th_mti_sc(gain_slider.val,cfar_mti_sc)[1]
    targets_rank = calc_th_mti_sc(gain_slider.val,cfar_mti_sc)[2]
...
ax.plot(getRank(signal_comp[1]),np.abs(signal_comp[1]),label='Comp $t_1$')
...
ax = axes[2]
ax.plot(getRank(cfar_mti_sc),
        np.abs(cfar_mti_sc),
        label='MTI Signal')

line_th_cfar_mti_sc, = ax.plot(getRank(calc_th_mti_sc(init_gain_mti,cfar_mti_sc)[0]),
                               np.abs(calc_th_mti_sc(init_gain_mti,cfar_mti_sc)[0]),
                               label='MTI Threshold')
ax.set_title('MTI SC')
ax.set_ylabel('Value')
ax.legend(loc='upper right')
...
ax = axes[3]
line_dif_th_mti_sc, = ax.plot(getRank(calc_th_mti_sc(init_gain_mti,cfar_mti_sc)[1]),
                              np.abs(calc_th_mti_sc(init_gain_mti,cfar_mti_sc)[1])
                              )

targets_rank = calc_th_mti_sc(init_gain_mti,cfar_mti_sc)[2]
